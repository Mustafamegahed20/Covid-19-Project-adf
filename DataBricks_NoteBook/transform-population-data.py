...
df_row_population = spark.read.csv("/mnt/covids19reportdl/row/population", sep=r'\t', header=True)
df_row_population = df_row_population.withColumn('age_group', regexp_replace(split(df_row_population['indic_de,geo\\time'], ',')[0], 'PC_', '')).withColumn('country_code', split(df_row_population['indic_de,geo\\time'], ',')[1])
df_row_population = df_row_population.select(col("country_code").alias("country_code"),
                                             col("age_group").alias("age_group"),
                                             col("2019 ").alias("percentage_2019"))
df_row_population.createOrReplaceTempView("row_population")
...
# Create a data frame with pivoted percentages
df_row_population_pivot = (
    spark.sql(
        "SELECT country_code, age_group, cast(regexp_replace(percentage_2019, '[a-z]', '') AS decimal(4,2)) AS percentage_2019 FROM row_population WHERE length(country_code) = 2"
    )
...
df_row_population_pivot.createOrReplaceTempView("row_population_pivot")
...
df_dim_country = spark.read.csv("/mnt/covids19reportdl/lookup/country-lookup", sep=r',', header=True)
...
df_processed_population.write.format("com.databricks.spark.csv").option("header","true").option("delimiter", ",").mode("overwrite").save("/mnt/covids19reportdl/processed/population")
# COMMAND ----------
